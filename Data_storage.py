...
def load_custom_env(username):
    ...
    # Si no, imprime un error
        ...
        raise FileNotFoundError(f"No se encontro el archivo {env_file}.")
...
class Snowflake:
    ...
    def extract_files_from_stage(self, stage):
        ...
        try:
            ...
            if not files:
                print("No se encontraro archivos en el stage.")
                return []
            ...
            for file in files:
                ...
                print(f"{len(data)} filas leídas del archivo {file_name}")

                # Verificar si los datos contienen records
                if data.empty:
                    print(f"No se encontraron datos en {file_name}.")
                    continue
            ...
            if combined_data.empty:
                print("El dataframe combinado esta vacío.")
                return []

            # Mensaje indicador del proceso
            print(f"Se combinaron los archivos del stage en uno solo con {len(combined_data)} filas.")
            return combined_data

        except Exception as e:
            print(f"Error extrayendo y combinando los archivos: {e}")
            return None
